...
func add_collision_polygons_via_api(path: String):
	print("\n🧱 Adding collision via TileData API...")
	...
	for i in range(tiles.size()):
		...
		var td: TileData = source.get_tile_data(atlas_coords, 0)
		if td == null:
			print("  ⚠️  No TileData at %s, skipping collision" % atlas_coords)
			continue
		var poly_idx = 0
	...
	if err == OK:
		print("  ✅ Collision added to %d tiles, re-saved %s" % [tiles_done, path])
	else:
